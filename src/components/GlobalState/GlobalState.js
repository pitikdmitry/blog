...
export const GlobalStateContextBackend = React.createContext({
  cursor: 0, /* Which page infinite scroll should fetch next. */
  useInfiniteScroll: true, /* Fallback in case of error. */
  items: [],
  isLoading: true,
...
export class GlobalStateBackend extends React.Component {
  ...
  getPath = () => {
    return `${__PATH_PREFIX__}/paginationJson/backend/index${this.state.cursor}.json`
  }
  ...
  render() {
    return (
      <GlobalStateContextBackend.Provider value={this.state}>
        {this.props.children}
      </GlobalStateContextBackend.Provider>
    )
  }
}
